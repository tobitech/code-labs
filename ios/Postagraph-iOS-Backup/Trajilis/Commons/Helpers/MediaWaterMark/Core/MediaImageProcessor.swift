...
extension MediaProcessor {
    func processImageWithElements(item: MediaWatermarkItem, completion: @escaping ProcessCompletionHandler) {
        if item.filter != nil {
            ...
            filterProcessor.processImage(image: item.sourceImage.fixedOrientation(), completion: { [weak self] (success, finished, image, error) in
                if error != nil {
                    ...
                    let updatedMediaWatermarkItem = MediaWatermarkItem(image: image!)
                    updatedMediaWatermarkItem.add(elements: item.mediaElements)
                    self?.processItemAfterFiltering(item: updatedMediaWatermarkItem, completion: completion)
                }
            })
    ...
    func processItemAfterFiltering(item: MediaWatermarkItem, completion: @escaping ProcessCompletionHandler) {
