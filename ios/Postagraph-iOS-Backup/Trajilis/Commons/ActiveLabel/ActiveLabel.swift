...
open class ActiveLabel: UILabel {
    ...
    @IBInspectable open var mentionColor: UIColor = .white {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var mentionSelectedColor: UIColor? {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var hashtagColor: UIColor = .white {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        self.setHashAlpha(mutAttrString,char:"@")
        self.setHashAlpha(mutAttrString,char:"#")
    }
    func setHashAlpha(_ mutAttrString: NSMutableAttributedString,char:String) {
        let ranges = mutAttrString.string.ranges(subString: char)
        for rng in ranges {
            if (rng.location != NSNotFound) {
                mutAttrString.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor.appRed, range: rng)
            }
        }
    }
    ...
    fileprivate func updateAttributesWhenSelected(_ isSelected: Bool) {
        //return; // this methos is causing issue & chanign # & @ colors to white. returning it as we are not using selected states.
