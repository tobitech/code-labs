...
class AddLocationViewViewModel {
    ...
    private func reverseGeocode() {
        // 51.400592 // 5.525292168404508
        // 4.760970 // 7.059699305353369
        let location = CLLocation(latitude: 5.525292168404508, longitude: 7.059699305353369)
        
        _querying.accept(true)
        
        // Reverse Geocode location coordinates
        locationService.reverseGeocode(location: location) {[weak self] (locations, error) in
            self?._querying.accept(false)
            self?._locations.accept(locations)
            
            if let error = error {
                print("Unable to Reverse Geocode Coordinates (\(error))")
            }
        }
    }
