...
@IBDesignable
public class CircularLoaderView: UIView, CAAnimationDelegate {
    ...
    private enum Utility {
        ...
        static func colorLerp(value: CGFloat, minMax: (UIColor, UIColor)) -> UIColor {
            ...
            return UIColor(red: lerp(value: clampedValue, minMax: (r0, r1)),
                           green: lerp(value: clampedValue, minMax: (g0, g1)),
                           blue: lerp(value: clampedValue, minMax: (b0, b1)), alpha: lerp(value: clampedValue, minMax: (a0, a1)))
        }
    ...
    private var progressLayer: CircularLoaderViewLayer {
        // swiftlint:disable:next force_cast
        return layer as! CircularLoaderViewLayer
    }
    ...
    override public class var layerClass: AnyClass {
        return CircularLoaderViewLayer.self
    }
    ...
    private class CircularLoaderViewLayer: CALayer {
        ...
        override init(layer: Any) {
            ...
            guard let progressLayer = layer as? CircularLoaderViewLayer else {
                return
            }
        ...
        override func draw(in ctx: CGContext) {
            ...
            if !lerpColorMode && colorsArray.count > 1 {
                ...
                if colorsArray.isEmpty {
                    ...
                    let tValue = CGFloat(reducedAngle) / 360
                    let steps = colorsArray.count - 1
                    let step = 1 / CGFloat(steps)
                    for index in 1...steps {
                        let fValue = CGFloat(index)
                        if tValue <= fValue * step || index == steps {
                            let colorT = Utility.inverseLerp(value: tValue, minMax: ((fValue - 1) * step, fValue * step))
